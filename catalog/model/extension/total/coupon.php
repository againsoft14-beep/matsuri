...
class ModelExtensionTotalCoupon extends Model {
	public function getCoupon($code) {
		...
		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code) . "' AND ((date_start = '0000-00-00' OR date_start <= CURDATE()) AND (date_end = '0000-00-00' OR date_end >= CURDATE())) AND status = '1'");
		...
		if ($status) {
			return array(
				...
				'shipping'      => (int)$coupon_query->row['shipping'],
		...
		if (isset($this->session->data['coupon'])) {
			...
			if ($coupon_info) {
				...
				if ($coupon_info['shipping'] && isset($this->session->data['shipping_method'])) {
					...
					$discount_total += (float)$this->session->data['shipping_method']['cost'];
				}

				// Cap discount so it cannot exceed running order total. When this total extension
				// runs before shipping, $total['total'] does not yet include shipping; allow the
				// free-shipping portion of the discount up to the selected shipping cost.
				$max_discount = $total['total'];

				if ($coupon_info['shipping'] && isset($this->session->data['shipping_method']['cost'])) {
					$shipping_in_totals = false;

					foreach ($total['totals'] as $running_total) {
						if (!empty($running_total['code']) && $running_total['code'] == 'shipping') {
							$shipping_in_totals = true;

							break;
						}
					}

					if (!$shipping_in_totals) {
						$max_discount += (float)$this->session->data['shipping_method']['cost'];
					}
				}
